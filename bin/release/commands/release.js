...
async function runReleaseBranchCheckoutStep(
	...
	await runStep('Getting into the release branch', abortMessage, async () => {
		await git.checkoutRemoteBranch(gitWorkingDirectoryPath, releaseBranch);
...
async function runCherrypickBumpCommitIntoMasterStep(
	...
	await runStep(
		'Cherry-picking the bump commit into main',
		abortMessage,
		async () => {
			await askForConfirmation(
			...
			await git.pushBranchToOrigin(gitWorkingDirectoryPath, 'main');
		}
	);
}
...
async function releaseRC() {
	...
	log(
		...
		"In a few minutes, you'll be able to find the GitHub release here: " +
			formats.success(`${config.wpRepositoryReleasesURL}v${version}`) +
			'\n',
